...
class _RedirectOut(object):
    ...
    def get_captured_output(self):
        ...
        return ''.join(map(convert, self.step_messages))
...
def _validate_step_info(step_info):
    if step_info.step_group and step_info.treat_nested_steps_as_methods:
        raise GrailValidationException('Step logic disabling is not applicable for step groups')
    if not step_info.step_group and state.step_execution_started:
        raise GrailValidationException('Step is called from another step (without group): %s' %
...
def _execute(step_info):
    ...
    try:
        ...
        if not state.is_test_wrapped:
            ...
            raise
        if step_info.step_group:
            raise GrailValidationException('Unexpected exception from step group: %s' % inst)

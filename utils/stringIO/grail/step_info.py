...
class StepInfo(object):
    ...
    def _get_clean_params(self):
        ...
        if args:
            ...
            if args_def and args_def[0] == 'self':
                args = args[1:]
        return args, self.kwargs

    def _get_name_based_description(self):
        return ' '.join(self.function.func_name.split('_'))
    ...
    def _get_arguments_string(self):
        ...
        kw_arguments = ['{0}={1}'.format(k, v) for k, v in kwargs.items()]
        return ' (' + ', '.join(args + kw_arguments) + ')'
    ...
    def get_description(self, output, result, exception_instance):
        ...
        if self.log_output and output:
            message += ' -> ' + unicode_replace(output)
        if exception_instance:
            message += ': %s' % unicode_replace(exception_instance)
        return message

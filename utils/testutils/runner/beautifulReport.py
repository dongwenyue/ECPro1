...
from configures.paths import CURRENTPATH
...
class PATH(object):
    ...
    if sys.platform.startswith('win'):
        config_tmp_path = CURRENTPATH + r'\utils\runner\template'
    else:
        config_tmp_path = CURRENTPATH + '/utils/runner/template'


class MakeResultJson(object):
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename=None, log_path='.'):
        """
            生成测试报告,并放在当前运行路径下
        ...
        with open(override_path + self.filename, 'wb') as write_file:
            for item in body:
                if item.strip().startswith(b'var resultData'):
                    ...
                    item[1] = head + json.dumps(self.FIELDS)
                    item = ''.join(item).encode()
                    item = bytes(item) + b';\n'
                write_file.write(item)

    @staticmethod
    def img2base(self, img_path, file_name):
        """
